...
(async()=>{
  ...
  await page.locator('#edit-value').fill('待定');
  await page.locator('#save-edit').click();
  await check('#edit-dialog [role=alert]','请输入完整');
  assert.equal(await page.locator('#edit-dialog').isVisible(),true);
  assert.equal(await page.locator('#edit-value').inputValue(),'待定');
  await check('#conflict-count','1');
  ...
  console.log('Headless review E2E passed: adopt, invalid-edit feedback, edit, undo, skip, authorize, content check, four widths, 6000-fact paging, loading-time navigation and failed-update notice.');
