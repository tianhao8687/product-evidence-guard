...
项目一路如何演进、为什么调整方向、测试说明了什么，见[项目迭代轨迹与复盘](docs/PROJECT_JOURNEY.md)。

本次交付：[稳定性、交互闭环与新原件验证](docs/STABLE_DELIVERY_20260917.md)。模型资源不足时保留可读取结果；采用、修改、撤销与导出使用同一份事实。文字 AI 辅助保留为可选，默认关闭，图片识别不受影响；没有把重复重读开启为默认功能。

此前简单题修复：[英文单位与数值在前的短参数行](docs/SIMPLE_FIXES_20260917.md)。同一批简单合成资料复测由 204/219 提升到 219/219，60/60 个格式变体通过；这是已暴露样本回归，不代表生产总体正确率 100%。

复杂资料检查：[原件读取改进与新增 10 份原件测试](docs/READING_REGIONS_20260916.md)。新增 101 页官方原件、80 个预登记检查点；完整正确自动采用覆盖由 17/80 提升到 26/80，仍有明显漏读和假冲突，尚未生产放行。该指标不是全部输出准确率。
...
新增 100 个资料包，覆盖 10 个硬件/工业细分领域：20 个公开资料改编包、80 个虚构
压力包，分别记录来源、标准答案和整包结果。它们不等于 100 份真实客户原件盲测。
原始 PDF 追加测试仍发现复杂表格的漏读与归属问题，暂不能以结构化包通过宣称全面可靠。
完整结果、根因修复与重构影响见[跨领域测试报告](docs/CROSS_DOMAIN_TEST_REPORT_20260916.md)。

## 简单处理入口（Unreleased）

审核页默认只展开需要处理的参数，提供“采用”“修改”“本次不使用”和“撤销上一步”。
普通修改只填正确值，说明选填；需要时才选择商品或口径。原始资料保持只读，人工修改单独追溯。
宿主对话可用 `review-action` 接受用户直接改值，也可提供一个本地“处理 / 修改参数”入口。

自动确认参数与人工确认参数均可导出、生成本地简报，用户一次授权后即可用于内容生成，
不再重复逐条批准。默认导出先检查是否完整；有未完成事项时，可明确选择 `--allow-partial` 仅导出已确认部分。严格人工范围仍可显式选择 `--mode human`。
未读完的资料另行提示，可重试并复用有效缓存；“没有冲突”不代表“资料全部读完”。

## 开放参数识别（Unreleased）

不再只接收固定字段。净重、电压等常见参数继续精确换算；“噪声：45 dB”“转速：3000 rpm”
“保修期：2年”等明确新参数直接保留原名称、完整值和来源，自动进入同一套审核、导出和回检流程。
新参数不会仅因不在注册表中就进入待确认；陌生单位不猜测换算，疑问才交给用户。

一行多个明确参数分别提取；表格的未知列不再丢弃。千位分隔、上下限、约数与公差保留，
产品尺寸与包装尺寸分开比较。密集 OCR 复核按批处理，读不完时明确提示，不把部分结果当成完整核验。
注册表仍用于熟悉参数的专业规则；文件安全、来源校验、人工决定与对外授权边界不变。

## v2 证据架构（Unreleased）

结果现在只分为 **冲突、待确认、已确认**。参数归属和含义清楚、没有不同值时，即使
只有一份资料也直接进入已确认；重复一致的记录合并展示，不要求逐项补依据。
不同口径分开处理，真正不同的值不会因为“多数资料一致”而被自动覆盖。
来源更新、商品归属不明或识别存疑才进入待确认；原文和来源默认收起，按需查看。

界面默认导出全部已确认参数，也可切换为仅人工批准参数。命令行使用
`export-table --mode verified` 导出全部已确认；省略模式也默认全部已确认，旧人工范围请显式传 `--mode human`。
自动确认不产生人工审计决定，也不自动授权向宿主发送资料。Excel 分为核验总览、
冲突、待确认事实、已确认事实和证据明细五张表，主表每个参数事实只出现一次。
本次同时更新了 Skill 操作说明；历史 Qoder 安装记录只证明当时版本的安装情况，
不代表新版说明已部署到已有的 Qoder 安装目录。

当前开发版把证据身份统一为 `source block + Product + Field + Scope + Unit + Value`，
同一资料目录现在可以安全处理多个 SKU/型号/变体。CSV、XLSX 和表格型 DOCX 会保留
稳定行边界并优先使用显式 SKU；没有 SKU 时才保守组合型号与变体，无法可靠绑定时
标记为歧义，不把跨商品差异误报成参数冲突。

字段与单位不再散落在代码常量中，而由经过严格 schema 校验的
`product_evidence_guard/field_registry.json` 驱动。注册表包含电气、机械、环境、
连接、合规和性能字段；注册表指纹进入分析与视觉缓存签名，规则变化会自动使旧缓存
失效。生成内容会先形成 `ClaimCandidate`，只有同 Product、Field、Scope、标准值和
单位均匹配当前授权事实时才标记 `supported`，不从容量推导续航，也不从参数推导
营销结论。

v2 收尾：`scope_aliases` 把“工作温度/储存温度”“额定扭矩/最大扭矩”等明确标签
映射为独立口径；`variant_headers` 指定可参与变体身份的表格列。不同规格独立核验，
同规格、同口径的不同值仍然报冲突。归属不明时先提示确认商品，不能仅通过确认数值
绕过归属核验并交付。普通文件预处理已计算的 SHA-256 会直接复用，避免重复读取。

本地审核页支持 Product/来源筛选、明确勾选后的批量确认或拒绝、冲突组的显式处理，
并在图片/PDF 有有效坐标时叠加证据框；没有可靠坐标时显示整页或整图并明确提示。
批量决定先整体校验，再原子发布状态，审计日志为每个候选保留共同事务号。确定性
文档预处理可并行运行，输出仍按稳定顺序聚合；OCR/Qwen 保持受控的单模型路径。

兼容性：既有 v1/v2 确认状态和 v1–v3 分析状态会在读取时升级；公开工作流仍可按原
方式单条确认、拒绝和导出。`product-facts.json` 与已确认事实输出升级为 schema 2，
消费者应忽略未知字段并使用 `product_id`、`scope` 和 `identity_version` 进行关联。

## 决赛工作流升级（开发版）

默认在 Qoder 对话内完成候选对比、参数确认、字段授权、内容回检和导出。用户使用 A1/A2 等选项即可；
本地证据页只在用户要求看原图/原文时提供，不自动打开、不作为必经入口。

云端额度不足或不可用时，可用 `export-local` 交付本地参数表和事实简报，无需模型/网络。
通信超时不会再触发客户端强制结束已有服务。当前 Qoder CLI 云端生成实测因返回额度不足而未完成。

新增 Qoder 后台任务、本地交互审核页、提前预热与演示常驻、按字段授权给宿主、
生成文案参数回检，以及交付件的事实引用/失效追踪。运行入口仍为 `scripts/run.ps1`。
操作方式和数据边界见 [内容闭环指南](docs/CONTENT_WORKFLOW.md)。下文的 2026 年 7 月实测是历史基线，
不代表本次新工作流的测试结果；用户已于 2026-09-04 确认既有 Qoder 接入完成验证。

本次还增加常驻模型内的跨任务图片缓存和仅参数视觉复核；上下文裁剪保留为默认关闭的开发实验，见
[速度优化与实测](docs/SPEED_OPTIMIZATION.md)。缓存重新绑定文件来源，不复用人工确认。
2026-09-05 收尾精简了 Skill 入口说明，修正排队状态与重分析期间的下一步提示，并在简短结果中显示本地耗时与模型复用状态。
...
核验推理使用本地 OpenVINO 和本地模型，不调用云端 OCR 或云端识别 API。内容生成可由 Qoder 使用用户明确授权的已确认字段完成；每次 `handoff` 都检查当前来源与授权。原始资料和完整证据报告保留在本地。项目脚本不主动调用云端生成 API，宿主的数据处理另行适用。
...
2026-08-25 最终 D 盘正式目录回归为 **265 项通过（84.114 s，0 跳过）**。Windows PowerShell
