...
日常操作更新：2026-09-17 · v2.0.1

把商品资料交给本机核对，处理真正有疑问的参数，然后导出。日常不需要理解“候选”、模型缓存或验收脚本。

## 平常只做这几步

1. 把本次资料放进一个文件夹，在已接入的宿主中说明“核对这个文件夹的商品参数”。多型号资料请保留明确型号和表头，不要把无关文件放进来。
2. 打开返回的“处理 / 修改参数”入口。默认只列需要处理的内容；上面的三张状态卡可以直接点选。
3. **冲突**：几个来源给出不同值，选一个采用；**待确认**：值、商品或条件还有疑问，点“修改”；**已确认**：默认无需处理。明确且无争议的单来源也能自动确认。
4. 需要核对时点“查看原文”。修改只填正确值，说明选填；原件不会改动，保存后可以撤销。参数多时直接搜索名称、商品或数值。
5. 点“导出参数表”。还有未完成事项时，页面会先问是否只导出已确认部分，导出的文件也会注明范围。

只有一个商品时不会要求反复选择商品。刷新、重新读取及“仅人工批准参数”放在“更多操作”中；来源和历史记录放在“更多筛选”中。

## 遇到问题怎么办

- **有资料未读完**：展开“查看未读完的资料”。复杂表格请补充清晰 CSV/XLSX 或原图，再重新读取；反复点重试通常不能解决同一个排版问题。
- **读取太久**：可以停止本次读取，再继续处理。不会结束其他程序；已经完成且原件未变的文件可以复用。
- **提示本地 AI 暂不可用**：文字和表格中能读出的结果会保留，未完成的图片/页面另行提示。在电脑空闲、内存充足时重试即可；不需要关闭工作软件或把资料上传云端。
- **来源更新了**：重新读取。旧决定会核对来源是否变化，不会直接套到新版本。
- **要让 WorkBuddy 写文案**：进入“授权与内容回检”，明确选择允许使用的参数，再检查生成文案。本地查看和导出不需要这个额外步骤。

当前仅供受控本地试运行。**“已确认”表示程序当前采信，不是保证绝对正确，也不表示整份资料已经读全。** 复杂原始 PDF 的跨行、型号和条件仍需留意；重要参数请打开原件核对。

升级后如果提示“单位读取规则已更新”，点击 **更新已有结果** 即可。原人工决定保留，只有读取结果变化的项目需要重新核对；不必把所有项目重新确认一遍。

日常不需要选择 AI 模式。文字疑难项的 AI 辅助目前是可选实验：明确想尝试时让宿主在分析命令加 `--semantic-assist`；关闭用 `--no-semantic-assist`。这两个开关都不影响正常的图片识别。AI 拿不准时仍需人工处理，重复回答不算额外证据。

安装和升级见[本地试运行手册](LOCAL_TRIAL_RUNBOOK.md)。下面保留进阶命令与早期运行记录，旧的逐候选确认描述不代表当前默认用户流程。

<details>
<summary>安装、命令行及历史验证记录（进阶）</summary>

以下历史观察主要记录于 2026-08-05，不作为当前版本的验收结论。
