...
0.438 秒返回 `running`。229 项回归覆盖 authkey、崩溃、重复启动和 timeout，但不是
渗透测试。
...
模型缓存后，三个离线/遥测环境变量下推理通过。最终回归 229 项通过
（74.951 s，0 跳过），唯一入口的 sidecar 确认、拒绝、导出、重分析与 stale E2E
...
真实图、synthetic Benchmark、离线变量、Qoder 调用和当前源目录 229 项回归均已
验证；历史 clean-room 为 219 项（77.233 s，0 跳过）。独立抓包、完整截图/视频和
