...
字段注册表是常见参数的处理规则，不是接收名单。明确的新标签和值会获得稳定的本地字段 ID，
在 `task.review_fields` 中返回名称和 ID，沿用现有授权、选项确认、导出和原文链接流程。
调用方使用工具返回的 ID，不自行拼接；不需要修改配置或逐项要求用户填写依据。
新参数的未知单位按原文保留，不猜测转换；新标签本身不触发待确认。

普通续航时间不强制要求用户补写“典型/最小/最大”；与其他明确口径无法比较时才提示疑问。
产品尺寸与包装尺寸分别显示，`≤300g` 不会被当成精确的 `300g`，`1,000g` 正确换算为 `1000g`。
内容回检会检查明确的新参数；无法对应的带数值声明显示未核对，不将其算作核对通过。
自然语言隐含关系、任意新字段的同义词、陌生单位换算仍不作无依据推断。

### 选项旁的原文链接

冲突和待确认选项均附“查看原文”：网页可直接展开对应证据；对话摘要通过 `source_url`
给出本机只读证据页；Excel 为每个选项/来源设置独立超链接，仍保持每个事实组一行。
多来源不会只链接第一份资料。证据页展示分析时的摘录、行号/页码/单元格位置，
来源校验有效时提供“打开来源文件”（PDF 带页码）。来源变化或无法访问时显示失效提示。

`source-links` 文件夹与报告一起保留；这些是本机链接，不是公网分享地址，换电脑需重新导出。
证据页是只读快照，不代表人工批准，也不会替代来源变化后的重新分析。
宿主只展示链接，不自动读取、抓取或上传链接中的原文。
