...
100 个跨领域资料包已建立并运行；组成、错误根因、回归与重构影响见
[测试报告](CROSS_DOMAIN_TEST_REPORT_20260916.md)。本阶段不以合成/改编包通过作为全面发布批准。

当前优先级：

1. **P0：原始 PDF 结构保留。** 先建立完整标注的型号矩阵、无框表、合并单元格、
   包装层级与条件表评测；将坐标/表头/父级条件转成既有结构化证据合同。
   保留旧解析回退与逐页覆盖提示，禁止静默把部分表格当整份资料已核验。
2. **P1：扩大原件与业务分布。** 新供应商、跨页/扫描/模糊/长表，以及当前 10 个
   硬件领域外的消费品资料；旧 holdout 已暴露，不再重复声称盲测。
3. **P1：用户处理成本。** 继续统计漏读、错误确认、假冲突、无必要待确认及处理次数；
   读取失败应在文档级给出简单入口，不把每个未读单元格变成用户待确认项。

以下为 2026-09-05 及以前的历史路线，不代表当前验收状态。

本次开发版已完成 Qoder 对话操作、常驻 CPU 推理、跨任务图片缓存、字段授权、内容回检和本地交付。
速度优化的证据及限制见 [速度记录](SPEED_OPTIMIZATION.md)，使用流程见 [内容闭环](CONTENT_WORKFLOW.md)。
默认关闭未证明真机收益的裁剪实验；后台排队与实际处理状态已区分，简短结果可查看耗时和模型复用。
决赛前由使用者完成现场流程排练与录屏；当前机器不进行 Intel GPU 验证。其他平台与 Intel Core Ultra 上的实测仍属于后续工作，不能用本机 CPU 结果代替。

以下内容保留为此前阶段的基线和发布路线，其中测试数量、耗时与发布包状态均对应各自日期，不代表当前开发目录的发布验收。
...
| 能力 | 状态 | 已有证据 | 仍需完成 |
|---|---|---|---|
| 证据记录、单位归一、冲突图、报告、增量缓存 | **已验证** | 2026-08-25 D 盘正式目录回归 265 项通过，84.114 s，0 跳过 | 继续扩展恶意/边界语料 |
...
状态：**已验证**。2026-08-25 最终 D 盘正式目录 Python 回归 265 项通过
（84.114 s，0 跳过）。
