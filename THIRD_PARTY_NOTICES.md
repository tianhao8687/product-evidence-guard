...
本仓库不内置模型权重或第三方 Python wheel。它们需要单独下载或安装，并继续受
各自许可证约束。若后续发行包捆绑 wheel、原生库或模型权重，发行者还必须一并
提供对应的上游许可证和声明文件。
...
固定 Windows/Python 3.11.13 环境中的 27 个直接与传递软件包及其分发哈希，但它
...
项目选择这条路线，是为了避免强 copyleft 的 PDF 渲染依赖；但最终收集二进制
声明仍属于发布阶段待办。
